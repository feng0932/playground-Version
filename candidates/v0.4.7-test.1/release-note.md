# v0.4.7-test.1 中文发版执行说明书

## 1. 这个版本做了什么

`v0.4.7-test.1` 是 `v0.4.7` 的第一次真实安装和控制链路测试版，不是正式稳定版。

这版要验证的不是“产品方法论已经完整成熟”，而是先把最关键的运行链路拿到真实证据：

1. 测试包能不能从发布入口被安装。
2. 安装后的项目里，版本号是不是 `v0.4.7-test.1`。
3. `/总控` 能不能生成真实派发。
4. 宿主能不能真的创建子线程。
5. 子线程能不能读取派发文件，而不是自己开聊。
6. 子线程能不能生成正式回执。
7. `00` 能不能读取回执并重新判断下一步。
...
```text
你现在要在本机实际执行 ai-team v0.4.7-test.1 的测试版新安装，不要只解释步骤，要真的运行命令，并把关键结果输出出来。
...
执行命令：
set -o pipefail
export AI_TEAM_RELEASE_METADATA_URL="http://192.168.1.152/yuhua/playground-Version/raw/branch/main/candidates/v0.4.7-test.1/ai-team-bundle-v0.4.7-test.1.release.json"
curl -fsSL http://192.168.1.152/yuhua/playground-Version/raw/branch/main/install-ai-team.sh | bash 2>&1 | tee /tmp/ai-team-install-v0.4.7-test.1.log

安装后继续执行：
export PATH="$HOME/.ai-team/bin:$PATH"
which ai-team
mkdir -p /tmp/ai-team-v0.4.7-test.1-smoke
cd /tmp/ai-team-v0.4.7-test.1-smoke
...
- /tmp/ai-team-install-v0.4.7-test.1.log 的关键片段
- 后续真实 session json / session jsonl 应保存到哪里

输出格式必须包含：
## ai-team v0.4.7-test.1 安装现场验收摘要
- 新安装是否完成：是 / 否
- 实际安装到的 bundle_version / release_tag：v0.4.7-test.1 / ai-team-bundle-v0.4.7-test.1
...
执行命令：
$env:AI_TEAM_RELEASE_METADATA_URL = "http://192.168.1.152/yuhua/playground-Version/raw/branch/main/candidates/v0.4.7-test.1/ai-team-bundle-v0.4.7-test.1.release.json"
$script = Join-Path $env:TEMP "install-ai-team-v0.4.7-test.1.ps1"
...
powershell -ExecutionPolicy Bypass -File $script *>&1 | Tee-Object -FilePath "$env:TEMP\\ai-team-install-v0.4.7-test.1.log"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

安装后继续执行：
$env:Path = "$HOME\\.ai-team\\bin;$env:Path"
where.exe ai-team
$smoke = Join-Path $env:TEMP "ai-team-v0.4.7-test.1-smoke"
...
- ai-team.lock.json 和 .ai-team\\runtime.json 的关键内容
- 是否出现 checksum mismatch、metadata fetch failure、downgrade block
- $env:TEMP\\ai-team-install-v0.4.7-test.1.log 的关键片段
- 后续真实 session json / session jsonl 应保存到哪里
...
```text
你现在要在本机实际执行 ai-team v0.4.7-test.1 的测试版更新，不要只解释步骤，要真的运行命令，并把更新前后结果对照输出。
...
set -o pipefail
export AI_TEAM_RELEASE_METADATA_URL="http://192.168.1.152/yuhua/playground-Version/raw/branch/main/candidates/v0.4.7-test.1/ai-team-bundle-v0.4.7-test.1.release.json"
curl -fsSL http://192.168.1.152/yuhua/playground-Version/raw/branch/main/install-ai-team.sh | bash 2>&1 | tee /tmp/ai-team-update-v0.4.7-test.1.log
...
- ai-team.lock.json 和 .ai-team/runtime.json 的关键内容
- 是否出现 checksum mismatch、metadata fetch failure、downgrade block
- /tmp/ai-team-update-v0.4.7-test.1.log 的关键片段
- 后续真实 session json / session jsonl 应保存到哪里
```
...
$script = Join-Path $env:TEMP "install-ai-team-v0.4.7-test.1.ps1"
Invoke-WebRequest http://192.168.1.152/yuhua/playground-Version/raw/branch/main/install-ai-team.ps1 -UseBasicParsing -OutFile $script
powershell -ExecutionPolicy Bypass -File $script *>&1 | Tee-Object -FilePath "$env:TEMP\\ai-team-update-v0.4.7-test.1.log"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
- ai-team.lock.json 和 .ai-team\\runtime.json 的关键内容
- 是否出现 checksum mismatch、metadata fetch failure、downgrade block
- $env:TEMP\\ai-team-update-v0.4.7-test.1.log 的关键片段
- 后续真实 session json / session jsonl 应保存到哪里
```
