...
PROJECT_PACKAGE_TARGETS_PATH = ROOT / "assets" / "contracts" / "project_package_formal_targets.json"
...
STRUCTURED_RESULT_REF_SUFFIXES = (
    "path",
    "anchor_or_section",
    "owner_agent",
    "task_chain_id",
...
PROJECT_PACKAGE_RESULT_REF_SUFFIXES = (
    "path",
    "owner_agent",
    "task_chain_id",
    "task_chain_epoch",
...
def load_project_package_formal_targets() -> list[str]:
    payload = load_json_mapping(PROJECT_PACKAGE_TARGETS_PATH, field_name="project_package_formal_targets")
    targets = payload.get("targets")
    if not isinstance(targets, list):
        raise ScriptFailure(
            exit_code=3,
            decision="schema_error",
            errors=["project_package_formal_targets targets must be a list"],
        )
    ...
    for entry in targets:
        ...
        if isinstance(target, str) and target.strip():
            formal_targets.append(target.strip())
    if not formal_targets:
        raise ScriptFailure(
            exit_code=3,
            decision="schema_error",
            errors=["project_package_formal_targets must define project_target entries"],
        )
...
def validate_cr004_receipt_fields(receipt_payload: dict[str, Any], *, project_root: object) -> None:
    if receipt_payload.get("return_reason") == "user_interrupt_rejudge":
        return
    child_thread = receipt_payload.get("child_thread")
    errors: list[str] = []
    if child_thread == "01-编排-初始化项目包":
        if is_project_package_authority_blocker_receipt(receipt_payload):
            errors.extend(validate_project_package_authority_blocker_receipt(receipt_payload))
        elif is_project_package_initialization_receipt(receipt_payload):
            errors.extend(validate_project_package_initialization_receipt(receipt_payload, project_root=project_root))
        else:
            errors.extend(
                validate_required_result_and_gate_status(
                    ...
                    expected_ref_owner="01-编排-初始化项目包",
        ...
        errors.extend(
            validate_required_result_and_gate_status(
                ...
                ref_prefix="deepening_result_ref",
                expected_ref_owner="10-执行-产品专家",
                project_root=project_root,
            )
        )
    if errors:
...
def is_project_package_authority_blocker_receipt(receipt_payload: dict[str, Any]) -> bool:
    return (
        receipt_payload.get("child_thread") == "01-编排-初始化项目包"
        and receipt_payload.get("return_reason") == "blocked"
        and receipt_payload.get("blocker_code") == "insufficient_project_package_write_authority"
    )


def validate_project_package_authority_blocker_receipt(receipt_payload: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if receipt_payload.get("recommended_return_target") != NO_PRIMARY_WINDOW_NEXT_HOP:
        errors.append("insufficient_project_package_write_authority must return no_primary_window_next_hop")
    if receipt_payload.get("project_package_ready") is not False:
        errors.append("insufficient_project_package_write_authority must set project_package_ready=false")
    if not normalize_target_list(receipt_payload.get("missing_formal_targets")):
        errors.append("insufficient_project_package_write_authority must include missing_formal_targets")
    if not isinstance(receipt_payload.get("formal_writable_targets"), list):
        errors.append("insufficient_project_package_write_authority must include formal_writable_targets")
    return errors


def is_project_package_initialization_receipt(receipt_payload: dict[str, Any]) -> bool:
    if receipt_payload.get("completed_scope") == "project_package_initialization":
        return True
    return any(
        is_meaningful_receipt_value(receipt_payload.get(field_name))
        for field_name in (
...
def validate_project_package_initialization_receipt(
    receipt_payload: dict[str, Any],
    *,
    project_root: object,
) -> list[str]:
    errors: list[str] = []
    if any(
        is_meaningful_receipt_value(receipt_payload.get(f"screening_result_ref_{suffix}"))
        for suffix in STRUCTURED_RESULT_REF_SUFFIXES
    ):
        errors.append("project_package_initialization must not use screening_result_ref_*")
    if is_meaningful_receipt_value(receipt_payload.get("screening_result_ref")):
        errors.append("project_package_initialization must not use screening_result_ref")

    required_targets = normalize_target_list(receipt_payload.get("required_formal_targets"))
    authorized_targets = normalize_target_list(receipt_payload.get("formal_targets_write_authorized"))
    written_targets = normalize_target_list(receipt_payload.get("formal_targets_written"))
    existing_targets = normalize_target_list(receipt_payload.get("formal_targets_existing"))
    missing_targets = normalize_target_list(receipt_payload.get("formal_targets_missing"))
    ready_value = receipt_payload.get("project_package_ready")
    required_contract_targets = load_project_package_formal_targets()

    if required_targets != required_contract_targets:
        errors.append("required_formal_targets must equal project package formal target contract")
    if authorized_targets != required_contract_targets:
        errors.append("formal_targets_write_authorized must include all required_formal_targets")
    if ready_value is not True and ready_value is not False:
        errors.append("project_package_ready must be true or false")
    if ready_value is False and receipt_payload.get("recommended_return_target") == "10-执行-产品专家":
        errors.append("project_package_ready false cannot recommend 10")
    if missing_targets and receipt_payload.get("recommended_return_target") == "10-执行-产品专家":
        errors.append("formal_targets_missing cannot recommend 10")
    if ready_value is True and missing_targets:
        errors.append("project_package_ready cannot be true when formal_targets_missing is non-empty")
    if ready_value is True:
        covered_targets = set(written_targets) | set(existing_targets)
        uncovered = [target for target in required_contract_targets if target not in covered_targets]
        if uncovered:
            errors.append("project_package_ready true requires formal_targets_written or existing cover all targets")

    errors.extend(
        validate_project_package_result_ref(
            receipt_payload,
            project_root=project_root,
            required=ready_value is True,
        )
    )
    return errors


def validate_project_package_result_ref(
    receipt_payload: dict[str, Any],
    *,
    project_root: object,
    required: bool,
    ...
    errors: list[str] = []
    values = {
        suffix: receipt_payload.get(f"project_package_result_ref_{suffix}")
        for suffix in PROJECT_PACKAGE_RESULT_REF_SUFFIXES
    }
    for suffix, value in values.items():
        if required and not is_meaningful_receipt_value(value):
            errors.append(f"missing structured result ref field: project_package_result_ref_{suffix}")
    if errors or not required:
        return errors

    owner = str(values["owner_agent"]).strip()
    if owner != "01-编排-初始化项目包":
        errors.append(f"project_package_result_ref_owner_agent mismatch: {owner} != 01-编排-初始化项目包")
    if str(values["task_chain_id"]).strip() != str(receipt_payload.get("task_chain_id") or "").strip():
        errors.append("project_package_result_ref_task_chain_id mismatch")
    if str(values["task_chain_epoch"]).strip() != str(receipt_payload.get("task_chain_epoch") or "").strip():
        errors.append("project_package_result_ref_task_chain_epoch mismatch")
    if str(values["gate_status"]).strip() != "project_package_ready":
        errors.append("project_package_result_ref_gate_status must be project_package_ready")
    if str(values["canonicalization"]).strip() != "canonical_json_utf8_bytes":
        errors.append("project_package_result_ref_canonicalization_mismatch")

    ref_path_value = str(values["path"]).strip()
    if Path(ref_path_value).is_absolute():
        errors.append("project_package_result_ref_path must be relative, not absolute")
        return errors
    ref_path = Path(ref_path_value)
    if any(part == ".." for part in ref_path.parts):
        errors.append("project_package_result_ref_path must not contain path traversal")
        return errors
    if "00-沟通记录与结论回写.md" in ref_path.parts:
        errors.append("project_package_result_ref_must_point_to_stable_json_carrier")
        return errors
    if ref_path.suffix != ".json":
        errors.append("project_package_result_ref_must_point_to_stable_json_carrier")
        return errors
    if not isinstance(project_root, str) or not project_root.strip():
        errors.append("project_root is required to validate project_package_result_ref_path")
        return errors
    ...
    try:
        ...
        errors.append("project_package_result_ref_path resolved outside project root")
        return errors
    if not candidate.is_file():
        errors.append(f"project_package_result_ref_path file does not exist: {ref_path_value}")
        return errors
    try:
        result_payload = json.loads(candidate.read_text(encoding="utf-8"))
    except OSError as exc:
        errors.append(f"project_package_result_ref_path file could not be read: {exc}")
        return errors
    except json.JSONDecodeError as exc:
        errors.append(f"project_package_result_ref_path file is not valid JSON: {exc}")
        return errors
    ...
    if str(values["sha256"]).strip() != actual_sha256:
        errors.append("project_package_result_ref_sha256_mismatch")
    return errors
...
def validate_01_to_10_execution_structure(receipt_payload: dict[str, Any]) -> None:
    if is_project_package_initialization_receipt(receipt_payload):
        return
    if not (
        receipt_payload.get("child_thread") == "01-编排-初始化项目包"
...
def validate_required_result_and_gate_status(
    ...
    *,
    ...
    result_is_meaningful = is_meaningful_receipt_value(result_value)
    ref_errors = validate_structured_result_ref(
        receipt_payload,
        ref_prefix=ref_prefix,
        expected_owner=expected_ref_owner,
    ...
    elif gate_value == complete_status and result_is_meaningful:
        errors.extend(
...
def validate_structured_result_ref(
    receipt_payload: dict[str, Any],
    *,
    ref_prefix: str,
    expected_owner: str,
    ...
    errors: list[str] = []
    if is_meaningful_receipt_value(receipt_payload.get(ref_prefix)):
        errors.append(f"{ref_prefix} single-line path#anchor ref is not consumable; use structured {ref_prefix}_* fields")
    if not required:
        return errors

    values = {
        suffix: receipt_payload.get(f"{ref_prefix}_{suffix}")
        for suffix in STRUCTURED_RESULT_REF_SUFFIXES
    }
    for suffix, value in values.items():
        if not is_meaningful_receipt_value(value):
            errors.append(f"missing structured result ref field: {ref_prefix}_{suffix}")
    if errors:
        return errors

    owner = str(values["owner_agent"]).strip()
    if owner != expected_owner:
        errors.append(f"{ref_prefix}_owner_agent mismatch: {owner} != {expected_owner}")

    task_chain_id = str(values["task_chain_id"]).strip()
    if task_chain_id != str(receipt_payload.get("task_chain_id") or "").strip():
        errors.append(f"{ref_prefix}_task_chain_id mismatch")

    task_chain_epoch = str(values["task_chain_epoch"]).strip()
    if task_chain_epoch != str(receipt_payload.get("task_chain_epoch") or "").strip():
        errors.append(f"{ref_prefix}_task_chain_epoch mismatch")

    gate_status = str(values["gate_status"]).strip()
    if gate_status != str(receipt_payload.get(gate_field) or "").strip():
        errors.append(f"{ref_prefix}_gate_status mismatch")

    ref_path_value = str(values["path"]).strip()
    if Path(ref_path_value).is_absolute():
        errors.append(f"{ref_prefix}_path must be relative, not absolute")
        return errors
    ref_path = Path(ref_path_value)
    if any(part == ".." for part in ref_path.parts):
        errors.append(f"{ref_prefix}_path must not contain path traversal")
        return errors
    if not isinstance(project_root, str) or not project_root.strip():
        errors.append(f"project_root is required to validate {ref_prefix}_path")
        return errors
    root = Path(project_root).expanduser().resolve()
    candidate = (root / ref_path).resolve()
    try:
        ...
        errors.append(f"{ref_prefix}_path resolved outside project root")
        return errors
    if not candidate.is_file():
        errors.append(f"{ref_prefix}_path file does not exist: {ref_path_value}")
        return errors
    try:
        artifact_text = candidate.read_bytes().decode("utf-8")
    except OSError as exc:
        errors.append(f"{ref_prefix}_path file could not be read: {exc}")
        return errors
    except UnicodeDecodeError as exc:
        errors.append(f"{ref_prefix}_path file is not UTF-8 text: {exc}")
        return errors
    normalized_text = artifact_text.replace("\r\n", "\n").replace("\r", "\n")

    anchor = str(values["anchor_or_section"]).strip()
    if anchor not in normalized_text:
        errors.append(f"{ref_prefix}_anchor_or_section not found: {anchor}")

    expected_sha256 = str(values["sha256"]).strip()
    actual_sha256 = hashlib.sha256(normalized_text.encode("utf-8")).hexdigest()
    if expected_sha256 != actual_sha256:
        errors.append(
            f"{ref_prefix}_sha256 mismatch; expected sha256(normalized_text.encode(\"utf-8\")) "
            "over normalized UTF-8 text with CRLF/CR normalized to LF"
        )
    return errors
...
def main(argv: list[str]) -> int:
    try:
        ...
        validate_cr004_receipt_fields(receipt_payload, project_root=project_root)
