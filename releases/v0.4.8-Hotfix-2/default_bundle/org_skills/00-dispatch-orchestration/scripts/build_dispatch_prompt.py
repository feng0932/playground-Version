...
from runtime_prompt_materialization import materialize_role_prompt_body
...
SUPPORTED_OPTIONAL_HANDOFF_SPEC_KEYS = {
    ...
    "receipt_contract",
    "blocker_scope",
    "formal_writable_targets",
}
REGISTRY_OWNED_OPTIONAL_HANDOFF_SPEC_KEYS = {
    "dod",
    "receipt_contract",
    "blocker_scope",
}
...
AUTHORITY_HOST = "00-项目包/04-过程文件/00-沟通记录与结论回写.md"
...
def load_handoff_spec(path: Path) -> dict[str, Any]:
    ...
    if "formal_writable_targets" in payload:
        formal_writable_targets = payload["formal_writable_targets"]
        if not isinstance(formal_writable_targets, list):
            fail("handoff spec rejected invalid formal_writable_targets: must be a list of non-empty strings")
        normalized_formal_targets: list[str] = []
        for index, target in enumerate(formal_writable_targets):
            if not isinstance(target, str):
                fail(f"handoff spec rejected invalid formal_writable_targets member at index {index}: must be a string")
            normalized_target = target.strip()
            if not normalized_target:
                fail(f"handoff spec rejected invalid formal_writable_targets member at index {index}: empty value")
            normalized_formal_targets.append(normalized_target)
        if not normalized_formal_targets:
            fail("handoff spec rejected invalid formal_writable_targets: must not be empty")
        payload["formal_writable_targets"] = normalized_formal_targets
    ...
    for optional_key in sorted(SUPPORTED_OPTIONAL_HANDOFF_SPEC_KEYS):
        if optional_key not in payload:
            continue
        if optional_key == "formal_writable_targets":
            continue
...
def validate_registry_owned_contract_fields(
    ...
    for key in sorted(REGISTRY_OWNED_OPTIONAL_HANDOFF_SPEC_KEYS):
...
def canonical_role_prompt_body(role_prompt_path: Path) -> tuple[dict[str, Any], str, bytes, str]:
    ...
    body = materialize_role_prompt_body(body)
...
def build_dispatch_input_contract(
    handoff_spec: dict[str, Any],
    registry_entry: dict[str, Any],
) -> dict[str, Any]:
    ...
    source_artifacts = _string_list(input_package.get("source_artifacts"))
    source_artifacts.extend(_string_list(input_package.get("files")))
    if not source_artifacts:
        source_artifacts = [
            AUTHORITY_HOST,
            str(handoff_spec["role_prompt_source"]),
            DISPATCH_CONTRACT_REGISTRY_SOURCE,
        ]

    current_blockers = _string_list(input_package.get("current_blockers"))
    blocking_issues = input_package.get("blocking_issues")
    if isinstance(blocking_issues, list):
        for issue in blocking_issues:
            if isinstance(issue, dict):
                reason = str(issue.get("reason") or issue.get("blocking_issue_id") or "").strip()
                if reason:
                    current_blockers.append(reason)
            elif str(issue).strip():
                current_blockers.append(str(issue).strip())

    expected_outputs = _string_list(input_package.get("expected_outputs"))
    if not expected_outputs:
        expected_outputs = [
            str(registry_entry["dod"]),
            str(registry_entry["receipt_contract"]),
        ]

    task_goal = str(input_package.get("goal") or handoff_spec["label"]).strip()
    current_layer = str(input_package.get("current_layer") or "product-usage-level").strip()
    if current_layer not in {"repo-level", "methodology-level", "product-usage-level"}:
        current_layer = "product-usage-level"
    ...
    contract = {
        ...
        "contract_version": "dispatch_input_contract.v1",
        "target_agent": handoff_spec["agent"],
        "task_goal": task_goal,
        "current_layer": current_layer,
        "authority_hosts": [AUTHORITY_HOST],
        "source_artifacts": source_artifacts,
        "read_what": source_artifacts,
        "current_blockers": current_blockers,
        "current_warnings": _string_list(input_package.get("current_warnings")),
        "allowed_actions": [
            "read_dispatch_contract_block",
            "read_role_prompt_body",
            "execute_assigned_scope_only",
            "return_machine_receipt",
        ],
        "forbidden_actions": [
            "expand_scope_without_blocker_receipt",
            "ask_user_directly_when_allow_direct_user_question_is_false",
            "skip_machine_receipt",
            "rewrite_dispatch_contract",
        ],
        "expected_outputs": expected_outputs,
        "required_receipt_fields": REQUIRED_FORMAL_RECEIPT_FIELDS,
        "completion_ceiling": "dispatch_request_materialized_not_live_verified",
        "return_target": "00-编排-总控 via /返回总控",
    }
    formal_writable_targets = _string_list(handoff_spec.get("formal_writable_targets"))
    if formal_writable_targets:
        contract["formal_writable_targets"] = formal_writable_targets
    return contract
...
def build_dispatch_contract_block(
    ...
    contract_block: dict[str, Any] = {
        ...
        **(
            {"formal_writable_targets": handoff_spec["formal_writable_targets"]}
        ...
        "dispatch_input_contract": build_dispatch_input_contract(handoff_spec, registry_entry),
    }
    ...
    for contract_key in (
        ...
        "shared_asset_foundation_contract",
        "module_layer_boundary_contract",
        "upper_truth_upgrade_contract",
    ):
...
def _validator_safe_result_example_lines(dispatch_intent: str) -> list[str]:
    return []


def build_machine_receipt_carrier(handoff_spec: dict[str, Any], registry_entry: dict[str, Any], role_prompt_sha256: str) -> str:
    ...
    receipt_lines = [
        "## Machine-readable format guard",
        "- 所有机器字段必须使用 `- field：value` 或 `- field: value` 的冒号格式；禁止使用 `field=value`。",
        "- 长材料必须外置到过程文件或正式 artifact，machine receipt 只写短摘要或结构化 ref 字段。",
        "- 禁止把 `screening_result` / `deepening_result` 长正文作为唯一机器消费依据。",
        "- `screening_result_ref_*` / `deepening_result_ref_*` 必须使用结构化字段组，不得写成 `path#anchor` 单行字段。",
        "- `human_confirmed` 与 `blocks_next_stage` 只能写 `true / false`，不得写中文、yes/no 或说明句。",
        "",
        "## 工单回执",
        "- read_what：",
        "- changed_what：",
    ...
    def append_machine_only_field(field_name: str, value: Any = "") -> None:
        normalized = str(field_name).strip()
        if not normalized or normalized in seen_receipt_fields:
            return
        if normalized in {"screening_result", "deepening_result"}:
            return
        receipt_lines.append(f"- {normalized}：{_receipt_value(value)}")
    ...
    if isinstance(stage_snapshot_contract, dict):
        for field_name in ("child_thread", "current_phase", "return_reason", "recommended_return_target"):
            value = stage_snapshot_contract.get(field_name, "")
            if field_name == "return_reason" and isinstance(value, list):
                value = ""
            append_machine_only_field(field_name, value)

    receipt_contract = registry_entry.get("receipt_contract")
    if isinstance(receipt_contract, dict):
        required_fields = receipt_contract.get("required_fields")
        if isinstance(required_fields, list):
            for field_name in required_fields:
                append_machine_only_field(str(field_name))

    receipt_lines.extend(
        [
            "",
            "## dispatch_evidence",
            f"- role_prompt_source：{handoff_spec['role_prompt_source']}",
            f"- role_prompt_sha256：{role_prompt_sha256}",
            "- dispatch_prompt_path：",
            "- dispatch_prompt_sha256：",
            f"- dispatch_instance_id：{handoff_spec['dispatch_instance_id']}",
            f"- task_chain_id：{handoff_spec['task_chain_id']}",
            f"- task_chain_epoch：{handoff_spec['task_chain_epoch']}",
        ]
    )

    footer_lines = [
        "`/返回总控` 是 child 退场动作，不得写入 `recommended_return_target`；"
        "`recommended_return_target` 只能写业务下一跳或 `no_primary_window_next_hop`，不能写 `00-编排-总控`。",
        "方案 C：有真实 hidden/file carrier 时优先写入 hidden/file carrier；没有真实 hidden/file carrier 时，用户输入 `/返回总控` 后必须在三句对人摘要之后追加 terminal machine receipt appendix。",
        "terminal machine receipt appendix 必须原样包含 `## 工单回执` 与 `## dispatch_evidence`；appendix 不属于普通 human-visible 3 句正文。",
        "该 machine receipt carrier 不得进入 human-visible reply body；terminal appendix 是没有真实 carrier 时的退场机器回执层，不属于普通 human-visible 3 句正文，不能由普通三句正文替代。",
    ]
    if handoff_spec.get("dispatch_intent") == "project_package_initialization":
        footer_lines.append(
            "`01 -> 10` 执行结构门：`next_10_writable_targets` 必须包含 "
            "`01-模块执行包/<模块名>/01-PRD/01-模块PRD.md`；`80-完整提交包/` 不能冒充 `10` 的 PRD 事实源。"
        )
...
def build_dispatch_prompt(
    ...
    if is_primary_window_registry_entry(registry_entry):
        sections.append(build_machine_receipt_carrier(handoff_spec, registry_entry, role_prompt_sha256))
    return "\n".join(sections)
...
def main(argv: list[str]) -> int:
    ...
    validate_registry_taxonomy(registry_entry)
