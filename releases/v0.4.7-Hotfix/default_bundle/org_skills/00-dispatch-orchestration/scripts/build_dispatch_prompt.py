...
def _validator_safe_result_example_lines(dispatch_intent: str) -> list[str]:
    if dispatch_intent == "project_package_initialization":
        return [
            "- screening_result 单行列表示例，必须替换为真实项目事实，不得原样提交：",
            "  - screening_item: example_item; phase: Phase A; question_asked: example_question; "
            "user_answer: example_answer; trigger_status: triggered; trigger_reason: example_reason; "
            "formal_hosts: 00-项目包/01-项目商业说明.md; handoff_to_10: example_handoff; "
            "human_confirmed: true; blocks_next_stage: false",
        ]
    if dispatch_intent == "product_source_completion":
        return [
            "- deepening_result 单行列表示例，必须替换为真实需求深挖结论，不得原样提交：",
            "  - source_screening_item: example_item; source_trigger_status: triggered; "
            "deepening_status: resolved; prd_hosts: 01-模块执行包/example/01-PRD/01-模块PRD.md; "
            "pages_states_paths: M-01,A-01; rules_or_mechanisms: example_rules; "
            "acceptance_criteria: example_acceptance; product_confirmation_items: example_confirmation; "
            "human_confirmed: true; blocks_next_stage: false",
        ]
    return []
...
def build_machine_receipt_carrier(handoff_spec: dict[str, Any], registry_entry: dict[str, Any], role_prompt_sha256: str) -> str:
    ...
    receipt_lines = [
        ...
        "- `screening_result` / `deepening_result` 必须写成字段下的一行字符串列表；禁止写成 YAML 对象块或 fenced JSON。",
        "- 列表项必须是单行 `key: value; key: value`，且同一列表项内必须包含全部 required fields。",
        "- `human_confirmed` 与 `blocks_next_stage` 只能写 `true / false`，不得写中文、yes/no 或说明句。",
        *_validator_safe_result_example_lines(str(handoff_spec.get("dispatch_intent", ""))),
        "",
        "## 工单回执",
        ...
        f"- allow_direct_user_question_used：{_receipt_value(handoff_spec['allow_direct_user_question'])}",
        f"- allowed_user_question_scope_used：{handoff_spec['allowed_user_question_scope']}",
        "- writable_targets_ack：",
        "- blocker_targets_touched：",
        "- closure_evidence_claims：",
    ]

    seen_receipt_fields: set[str] = set()
    ...
    if isinstance(stage_snapshot_contract, dict):
        for field_name, value in stage_snapshot_contract.items():
            append_machine_only_field(field_name, value)

    upper_truth_upgrade_contract = registry_entry.get("upper_truth_upgrade_contract")
    if isinstance(upper_truth_upgrade_contract, dict):
        required_receipt_fields = upper_truth_upgrade_contract.get("required_receipt_fields")
        if isinstance(required_receipt_fields, list):
