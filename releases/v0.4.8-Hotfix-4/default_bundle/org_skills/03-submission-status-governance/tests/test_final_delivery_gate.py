...
class FinalDeliveryGateUnitTests(unittest.TestCase):
    ...
    @staticmethod
    def _valid_lifecycle_evidence(module) -> dict[str, object]:
        return {
            "first_generated_artifact_verdict": "passed",
            "first_generated_artifact_evidence": "80-完整提交包/02-原型索引/00-原型入口.md",
            "first_freeze_artifact_verdict": "passed",
            "first_freeze_artifact_evidence": module.RECEIPT_PATH.as_posix() + "#4-原型审查记录",
            "remediation_verdict": "none",
    ...
    def _prepare_host_project(
        ...
        *,
        ...
        receipt = {
            ...
            "prototype_lifecycle_evidence": cls._valid_lifecycle_evidence(module),
        }
        receipt_path.write_text(
            "\n".join(
                [
    ...
    def test_validate_receipt_requires_matching_semantic_boundary(self) -> None:
        ...
        receipt = {
            ...
            "prototype_lifecycle_evidence": self._valid_lifecycle_evidence(module),
        }

        project_root = self._copy_minimal_project()
        with self.assertRaises(ValueError) as exc:
