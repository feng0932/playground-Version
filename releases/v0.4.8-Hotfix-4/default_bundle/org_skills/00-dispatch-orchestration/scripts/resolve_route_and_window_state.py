...
EXECUTION_STRUCTURE_QUESTION_SCOPE = "只允许追问当前 primary window 的执行结构缺口"
...
def normalize_project_package_authorized_targets(dispatch_request: dict[str, Any]) -> list[str]:
    authorized = [
        *normalize_target_list(dispatch_request.get("formal_writable_targets")),
        *normalize_target_list(dispatch_request.get("writable_targets")),
    ]
    seen: set[str] = set()
    normalized: list[str] = []
...
def validate_project_package_dispatch_authority(dispatch_request: dict[str, Any]) -> None:
    required_targets = load_project_package_formal_targets()
    authorized_targets = normalize_project_package_authorized_targets(dispatch_request)
    missing_targets = [target for target in required_targets if target not in authorized_targets]
    if not missing_targets:
        return
    raise ScriptFailure(
        exit_code=2,
        decision="dispatch_blocked",
        errors=["project_package_formal_targets_not_authorized"],
        result={
            "route_verdict": "dispatch_blocked",
            "dispatch_allowed": False,
            "dispatch_target": "01-编排-初始化项目包",
            "next_required_action": "repair_dispatch_contract_or_authority_carrier",
            ...
            "dispatch_authority": "project_package_formal_targets_not_authorized",
            "blocker_code": "project_package_formal_targets_not_authorized",
            "required_formal_targets": required_targets,
            "authorized_formal_targets": [
                target for target in authorized_targets if target in required_targets
            ],
            "missing_formal_targets": missing_targets,
            "prompt_text": None,
            "host_native_dispatch": {"required_tool": None},
        },
    )
...
def missing_execution_structure_fields(latest_status: dict[str, Any]) -> list[str]:
    missing = [
        field_name
        for field_name in EXECUTION_STRUCTURE_BOOLEAN_FIELDS
        if latest_status.get(field_name) is not True
    ]
    if not formal_module_prd_targets(latest_status.get("next_10_writable_targets")):
        missing.append("next_10_writable_targets")
    return missing


def retained_01_execution_structure_blocker(latest_status: dict[str, Any]) -> bool:
    if latest_status.get("receipt_consumed") is not True:
        return False
    if latest_status.get("child_thread") != "01-编排-初始化项目包":
        return False
    if latest_status.get("return_reason") != "blocked":
        return False
    if latest_status.get("recommended_return_target") != "no_primary_window_next_hop":
        return False
    if latest_status.get("project_package_ready") is not True:
        return False
    if normalize_target_list(latest_status.get("formal_targets_missing")):
        return False
    return bool(missing_execution_structure_fields(latest_status))


def apply_primary_window_question_policy(dispatch_request: dict[str, Any], *, dispatch_intent: str) -> None:
    if "allow_direct_user_question" not in dispatch_request:
        dispatch_request["allow_direct_user_question"] = dispatch_intent in PRIMARY_WINDOW_DISPATCH
    if "allowed_user_question_scope" not in dispatch_request:
        dispatch_request["allowed_user_question_scope"] = (
            "primary-window-intake"
            if parse_boolish(dispatch_request.get("allow_direct_user_question"))
            else "none"
        )


def apply_execution_structure_repair_dispatch(dispatch_request: dict[str, Any], latest_status: dict[str, Any]) -> None:
    dispatch_request["allow_direct_user_question"] = True
    dispatch_request["allowed_user_question_scope"] = EXECUTION_STRUCTURE_QUESTION_SCOPE
    dispatch_request["label"] = "派发01-编排-初始化项目包-执行结构修复"
    dispatch_request["input_package"] = {
        "goal": "Close the execution-structure gate before dispatching product source completion.",
        "source": SCRIPT_NAME,
        "current_blocker_code": "execution-directory-derivation-blocked",
        "current_blockers": ["execution-directory-derivation-blocked"],
        "project_package_ready": latest_status.get("project_package_ready") is True,
        "missing_execution_structure_fields": missing_execution_structure_fields(latest_status),
        "required_next_10_target_pattern": "01-模块执行包/<模块名>/01-PRD/01-模块PRD.md",
        "retained_project_package_result_ref_path": str(latest_status.get("project_package_result_ref_path") or ""),
    }
...
def build_closed_handoff_spec(
    *,
    ...
    apply_primary_window_question_policy(dispatch_request, dispatch_intent=dispatch_intent)
    allow_direct_user_question = parse_boolish(dispatch_request.get("allow_direct_user_question", False))
    ...
    if dispatch_intent == "project_package_initialization":
        handoff_spec["formal_writable_targets"] = load_project_package_formal_targets()
    return handoff_spec
...
def resolve_dispatch_authority(
    *,
    ...
    if dispatch_intent == "project_package_initialization":
        retained_10_verdict = retained_10_no_primary_next_hop_verdict(latest_status)
        if retained_10_verdict is not None:
            if retained_10_verdict[0] and retained_10_verdict[1] == "structure_repair_rejudge_ready":
                apply_execution_structure_repair_dispatch(dispatch_request, latest_status)
            return retained_10_verdict
        if retained_01_execution_structure_blocker(latest_status):
            validate_project_package_dispatch_authority(dispatch_request)
            apply_execution_structure_repair_dispatch(dispatch_request, latest_status)
            return True, "execution_structure_repair_rejudge_ready"
        validate_project_package_dispatch_authority(dispatch_request)
        return True, "dispatch_authorized"
    ...
    if (
        ...
        if latest_status.get("project_package_ready") is True and not normalize_target_list(
            latest_status.get("formal_targets_missing")
        ):
...
def retained_10_no_primary_next_hop_verdict(latest_status: dict[str, Any]) -> tuple[bool, str] | None:
    if latest_status.get("receipt_consumed") is not True:
        return None
    if latest_status.get("child_thread") != "10-执行-产品专家":
        return None
    if latest_status.get("recommended_return_target") != "no_primary_window_next_hop":
        return None
    return_reason = latest_status.get("return_reason")
    if return_reason == "completed":
        return False, "no_primary_window_dispatch_pending"
    if return_reason == "blocked":
        if (
            latest_status.get("structure_blocker_owner") == STRUCTURE_BLOCKER_OWNER
            and latest_status.get("structure_blocker_code") in STRUCTURE_BLOCKER_CODES
        ):
            return True, "structure_repair_rejudge_ready"
        return False, "structure_blocker_code_required"
    return False, "no_primary_window_dispatch_pending"
...
def main(argv: list[str]) -> int:
    try:
        ...
        next_required_action = (
            ...
            else "await_user_next_intent"
            if authority_verdict in {"no_primary_window_dispatch_pending", "structure_blocker_code_required"}
            else "/读取回执"
        )
        if authority_verdict == "no_primary_window_dispatch_pending":
            route_verdict = "no_primary_window_dispatch_pending"
        result: dict[str, Any] = {
        ...
        decision = (
            "dispatch_allowed"
            if dispatch_allowed
            else "no_primary_window_dispatch_pending"
            if authority_verdict == "no_primary_window_dispatch_pending"
