...
def load_handoff_spec(path: Path) -> dict[str, Any]:
    ...
    if "window_activation_snapshot" in payload:
        validate_window_activation_snapshot(payload["window_activation_snapshot"], expected_agent=agent)
...
def validate_window_activation_snapshot(snapshot: dict[str, Any], *, expected_agent: str) -> None:
    required_keys = {
    ...
    if missing_keys:
        fail("handoff spec rejected invalid window_activation_snapshot: missing keys: " + ", ".join(missing_keys))
    if snapshot.get("human_contact_allowed") is not True:
        fail("handoff spec rejected invalid window_activation_snapshot: human_contact_allowed must be true")
    if snapshot.get("human_contact_owner") != expected_agent:
        fail("handoff spec rejected invalid window_activation_snapshot: human_contact_owner must match agent")
    if snapshot.get("human_contact_mode") != "direct_primary_window":
        fail("handoff spec rejected invalid window_activation_snapshot: human_contact_mode must be direct_primary_window")
    if snapshot.get("window_state") != "active":
        fail("handoff spec rejected invalid window_activation_snapshot: window_state must be active")
    next_required_action = snapshot.get("next_required_action")
    if next_required_action not in {"stay_in_current_primary_window", "/返回总控"}:
        fail("handoff spec rejected invalid window_activation_snapshot: unsupported next_required_action")
...
def build_dispatch_contract_block(
    ...
    if "window_activation_snapshot" in handoff_spec:
        contract_block["window_activation_snapshot"] = handoff_spec["window_activation_snapshot"]
...
def build_machine_receipt_carrier(handoff_spec: dict[str, Any], registry_entry: dict[str, Any]) -> str:
    ...
    def append_machine_only_field(field_name: str) -> None:
